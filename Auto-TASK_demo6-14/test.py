...
# goal = input('what you want:')
    ...
    while num_plan < 3:
        ...
        if num_plan == 2:
            print(res['thoughts']['speak'])
            return f"{res['thoughts']['text']}{res['thoughts']['reasoning']}{res['thoughts']['plan']}{res['thoughts']['criticism']}{res['thoughts']['speak']}"
        prompt_generator.performance_evaluation.append(f"the command '{command_name}' with the arg '{arguments}' have finished, result {command_result}")
        num_plan = num_plan + 1
